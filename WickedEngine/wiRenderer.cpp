...
namespace GGTrees
{
	extern "C" void GGTrees_Draw_ShadowMap( const Frustum* frustum, int cascade, wiGraphics::CommandList cmd );
	extern "C" void __GGTrees_Draw_ShadowMap_EMPTY( const Frustum* frustum, int cascade, wiGraphics::CommandList cmd ) {}
	#pragma comment(linker, "/alternatename:GGTrees_Draw_ShadowMap=__GGTrees_Draw_ShadowMap_EMPTY")

	extern "C" void GGTrees_Draw_EnvProbe( const SPHERE* culler, const Frustum* frusta, uint32_t frustum_count, wiGraphics::CommandList cmd );
	extern "C" void __GGTrees_Draw_EnvProbe_EMPTY( const SPHERE* culler, const Frustum* frusta, uint32_t frustum_count, wiGraphics::CommandList cmd ) {}
	#pragma comment(linker, "/alternatename:GGTrees_Draw_EnvProbe=__GGTrees_Draw_EnvProbe_EMPTY")
}

namespace GGGrass
{
	extern "C" void GGGrass_Draw_ShadowMap( const Frustum* frustum, int cascade, wiGraphics::CommandList cmd );
...
uint32_t SHADOWCOUNT_2D = 5; //PE: Not needed anymore its now dynamic.
uint32_t SHADOWCOUNT_SPOT_2D = 3;
uint32_t SHADOWCOUNT_CUBE = 2;//(16) 5; give the universe what it wants :) //PE: its now dynamic so no need to allocate 16 on startup.
...
struct RenderBatch
{
	...
	inline void Create(size_t meshIndex, size_t instanceIndex, float _distance)
	{
		...
		hash |= (uint32_t)(meshIndex & 0x00FFFFFF);
#ifdef GGREDUCED
		//PE: As wicked is based on small distances, we devide by 500 to instancing batches more precise.
		//PE: Instanced objects has frustum culling before bastching, so safe to increase instanced batches. now 5000.
		hash |= ((uint32_t) (_distance/5000.0) & 0xFF) << 24;
#endif
...
#ifdef GGREDUCED
	wiJobSystem::Execute(ctx, [](wiJobArgs args) { LoadShader(CS, shaders[CSTYPE_POSTPROCESS_RAIN], "rainCS.cso"); });
	wiJobSystem::Execute(ctx, [](wiJobArgs args) { LoadShader(CS, shaders[CSTYPE_POSTPROCESS_SNOW], "snowCS.cso"); });
#endif
...
inline void CreateDirLightShadowCams(const LightComponent& light, CameraComponent camera, std::array<SHCAM, CASCADE_COUNT>& shcams)
{
...
#ifdef GGREDUCED
	float referenceFarPlane = fWickedCallShadowFarPlane; // cascade splits here were tested with this depth range
#else
	float referenceFarPlane = 800.0f; // cascade splits here were tested with this depth range
#endif
	referenceFarPlane = std::min(referenceFarPlane, farPlane); // if reference plane is greater than far plane, do not increase cascade sizes further
	const float referenceSplitClamp = referenceFarPlane / farPlane;
	...
	for (int cascade = 0; cascade < CASCADE_COUNT; ++cascade)
	{
		...
		//ext = std::max(ext, farPlane * 0.5f); //PE: check this , see discord. perhaps limit it "ext = std::max(ext, std::min(1500.0f, farPlane) * 0.5f);"
		//PE: If Orthographic matrix min.z,max.z do not reach all the way to our largest cascade range then cascades will begind to repeat.
#ifdef GGREDUCED
		if (fWickedMaxCenterTest <= 0.0)
		{
			//ext = std::max(ext, farPlane * 0.5f);
			ext = std::max(ext, farPlane * 0.65f); //PE: A little larger.
		}
		else
		{
			ext = fWickedMaxCenterTest;
		}
#else
		ext = std::max(ext, farPlane * 0.5f);
#endif
...
void BindShadowmaps(SHADERSTAGE stage, CommandList cmd)
{
	...
	device->BindResource(stage, &shadowMapArray_Spot_2D, TEXSLOT_SHADOWARRAY_SPOT_2D, cmd);
	if (GetTransparentShadowsEnabled())
	{
		...
		device->BindResource(stage, &shadowMapArray_Transparent_Spot_2D, TEXSLOT_SHADOWARRAY_TRANSPARENT_SPOT_2D, cmd);
	}
}
...
)
{
	if (renderQueue.empty())
	{
		...
		const int instanceBufferDescriptorIndex = device->GetDescriptorIndex(instances.buffer, SRV);
...
#endif
		{
			...
			for (int instancedBatchID = 0; instancedBatchID < instancedBatchCount; ++instancedBatchID)
			{
				...
				{
					const MaterialComponent& material = vis.scene->materials[mesh.subsets[0].materialIndex];
					if (iDoubleRender == 1 && material.GetBlendMode() != BLENDMODE_FORCEDEPTH)
						continue;
				}
#endif
...
)
{
	...
	{
		frameCB.g_xFrame_SunColor = XMFLOAT3(1, 1, 1);
		frameCB.g_xFrame_SunEnergy = 9.0;
	...
	{
		frameCB.g_xFrame_Fog = float4(900000.0, 1000000.0, vis.scene->weather.fogHeightStart, vis.scene->weather.fogHeightEnd);
	}
...
//#endif
	...
	frameCB.g_xFrame_ShadowKernel2D = SHADOWRES_2D ? 1.0f / SHADOWRES_2D : 0;
	frameCB.g_xFrame_ShadowKernelCube = SHADOWRES_CUBE ? 1.0f / SHADOWRES_CUBE : 0;

	frameCB.g_xFrame_ShadowResSpot2D = (float)SHADOWRES_SPOT_2D;
	frameCB.g_xFrame_ShadowKernelSpot2D = SHADOWRES_SPOT_2D ? 1.0f / SHADOWRES_SPOT_2D : 0;

	frameCB.g_xFrame_WaterColor.x = vis.scene->weather.oceanParameters.waterColor.x;
	frameCB.g_xFrame_WaterColor.y = vis.scene->weather.oceanParameters.waterColor.y;
	frameCB.g_xFrame_WaterColor.z = vis.scene->weather.oceanParameters.waterColor.z;
	frameCB.g_xFrame_WaterHeight = vis.scene->weather.oceanParameters.waterHeight;

	frameCB.g_xFrame_WaterFogMax = vis.scene->weather.oceanParameters.fogMaxDist;
	frameCB.g_xFrame_WaterFogMin = vis.scene->weather.oceanParameters.fogMinDist;
	frameCB.g_xFrame_WaterFogMinAmount = vis.scene->weather.oceanParameters.fogMinAmount;
	...
	{
		frameCB.g_xFrame_Options |= OPTION_BIT_WATER_ENABLED;
	}
...
)
{
	...
	// Fill Entity Array with decals + envprobes + lights in the frustum:
	{
		...
		for (size_t i = 0; i < std::min((size_t)vis.scene->envmapCount, vis.visibleEnvProbes.size()); ++i)
		{
			...
			entityArray[entityCounter].userdata = probe.userdata;
		...
		for (auto visibleLight : vis.visibleLights)
		{
			...
			else
			{
				...
				{
					entityArray[entityCounter].color = wiMath::CompressColor(XMFLOAT3(1, 1, 1));
					entityArray[entityCounter].SetEnergy(9.0f);
				}
				else
				{
					entityArray[entityCounter].color = wiMath::CompressColor(light.color);
			...
			else if(shadow)
			{
				...
				if (shadow)
				{
					...
					case LightComponent::DIRECTIONAL:
						...
						break;
					case LightComponent::SPOT:
						if (shadowCounter_Spot_2D < SHADOWCOUNT_SPOT_2D)
						{
							entityArray[entityCounter].SetIndices(matrixCounter, shadowCounter_Spot_2D);
...
//PE: No need for a second lookup.
//#ifdef GGREDUCED
//				const ArmatureComponent& armature = *vis.scene->armatures.GetComponent(mesh.armatureID);
//#endif
...
void SetShadowPropsSpot2D(int resolution, int count)
{
	if (resolution >= 0)
	{
		SHADOWRES_SPOT_2D = resolution;
	}
	if (count >= 0)
	{
		SHADOWCOUNT_SPOT_2D = count;
	}

	if (SHADOWCOUNT_SPOT_2D > 0 && SHADOWRES_SPOT_2D > 0)
	{
		TextureDesc desc;
		desc.Width = SHADOWRES_SPOT_2D;
		desc.Height = SHADOWRES_SPOT_2D;
		desc.MipLevels = 1;
		desc.ArraySize = SHADOWCOUNT_SPOT_2D;
		desc.SampleCount = 1;
		desc.Usage = USAGE_DEFAULT;
		desc.CPUAccessFlags = 0;
		desc.MiscFlags = 0;
		...
		desc.clear.color[2] = 1;
		desc.clear.color[3] = 0;
		device->CreateTexture(&desc, nullptr, &shadowMapArray_Transparent_Spot_2D);
#endif

		renderpasses_spot_shadow2D.resize(SHADOWCOUNT_SPOT_2D);

		for (uint32_t i = 0; i < SHADOWCOUNT_SPOT_2D; ++i)
		{
			int subresource_index;
			subresource_index = device->CreateSubresource(&shadowMapArray_Spot_2D, DSV, i, 1, 0, 1);
			assert(subresource_index == i);
#ifdef ENABLE_TRANSPARENT_SHADOWS
			if (GetTransparentShadowsEnabled())
			{
				subresource_index = device->CreateSubresource(&shadowMapArray_Transparent_Spot_2D, RTV, i, 1, 0, 1);
				assert(subresource_index == i);
			}
#endif
			...
			if (GetTransparentShadowsEnabled())
			{
				renderpassdesc.attachments.push_back(
					RenderPassAttachment::RenderTarget(
						&shadowMapArray_Transparent_Spot_2D,
						RenderPassAttachment::LOADOP_CLEAR,
						RenderPassAttachment::STOREOP_STORE,
						IMAGE_LAYOUT_SHADER_RESOURCE,
						IMAGE_LAYOUT_RENDERTARGET,
			...
			device->CreateRenderPass(&renderpassdesc, &renderpasses_spot_shadow2D[subresource_index]);
		}
	}
...
)
{
	...
	if (!vis.visibleLights.empty())
	{
		...
		device->UnbindResources(TEXSLOT_SHADOWARRAY_SPOT_2D, 1, cmd);
		...
		for (const auto& visibleLight : vis.visibleLights)
		{
			...
			//PE: Delayed shadows , cascade fixed. cascade4 (every 4 frames) cascade3 (every 3 frames) cascade2 (every 2 frames) cascade0-1 always.
			//PE: Point , base this on distance to camera.
			//PE: Spot , base this on distance to camera.
			//PE: To be adjusted depending on result.

			switch (light.GetType())
			{
			case LightComponent::DIRECTIONAL:
			{
				if (shadowCounter_2D >= SHADOWCOUNT_2D - CASCADE_COUNT + 1 || SHADOWCOUNT_2D == 0)
					break;
					...
					#endif
					{
						...
						if ( cascade < 3 ) // skip wicked engine objects in last 2 cascades
						{
							for (size_t i = 0; i < vis.scene->aabb_objects.GetCount(); ++i)
							{
								...
								if ( cascade == 2 ) // skip small objects in third cascade
								{
									float sizeX = aabb._max.x - aabb._min.x;
									float sizeY = aabb._max.y - aabb._min.y;
									float sizeZ = aabb._max.z - aabb._min.z;
									...
									float areaXZ = sizeX * sizeZ;
									float areaYZ = sizeY * sizeZ;
									if ( areaXY < 38000 && areaXZ < 38000 && areaYZ < 38000 ) continue; // about 5m x 5m
								}
						...
						if (!g_bNoTerrainRender)
						{
							...
							sprintf_s(profileName, "Shadow Rendering - Terrain %d", cascade);
							auto range2 = wiProfiler::BeginRangeGPU(profileName, cmd);
							GGTerrain::GGTerrain_Draw_ShadowMap(&shcams[cascade].frustum, cascade, cmd);
							...
							if (cascade < 3)
							{
								...
								GGTrees::GGTrees_Draw_ShadowMap(&shcams[cascade].frustum, cascade, cmd);
								wiProfiler::EndRange(range2);
							}

							//PE: Make more room. grass dont produce shadows.
							//sprintf_s(profileName, "Shadow Rendering - Grass %d", cascade);
							//range2 = wiProfiler::BeginRangeGPU(profileName, cmd);
							//GGGrass::GGGrass_Draw_ShadowMap(&shcams[cascade].frustum, cascade, cmd);
							//wiProfiler::EndRange(range2);
						}
			...
			case LightComponent::SPOT:
			{
				...
				if (!renderQueue.empty())
				{
					...
					vp.TopLeftX = 0;
					vp.TopLeftY = 0;
					vp.Width = (float)SHADOWRES_SPOT_2D;
					vp.Height = (float)SHADOWRES_SPOT_2D;
					vp.MinDepth = 0.0f;
					vp.MaxDepth = 1.0f;
					device->BindViewports(1, &vp, cmd);

					device->RenderPassBegin(&renderpasses_spot_shadow2D[slice], cmd);
					RenderMeshes(vis, renderQueue, RENDERPASS_SHADOW, RENDERTYPE_OPAQUE, cmd);
					if (GetTransparentShadowsEnabled() && transparentShadowsRequested)
					{
						RenderMeshes(vis, renderQueue, RENDERPASS_SHADOW, RENDERTYPE_TRANSPARENT | RENDERTYPE_WATER, cmd);
...
//	const bool occlusion = flags & DRAWSCENE_OCCLUSIONCULLING;
	...
	if (!renderQueue.empty())
	{
		...
		//PE: https://github.com/turanszkij/WickedEngine/commit/f49ecdb60e40decb4eb559f986458f68a5c93cbf
		//PE: Sorting only needed for transparent and prepass.
		if (transparent)
		{
			renderQueue.sortdistance(RenderQueue::SORT_BACK_TO_FRONT);
		}
...
void RefreshEnvProbes(const Visibility& vis, CommandList cmd)
{
	...
	auto render_probe = [&](const EnvironmentProbeComponent& probe, const AABB& probe_aabb) {
		...
		if (probe_aabb.layerMask & vis.layerMask)
		{
			...
			GGTerrain::GGTerrain_Draw_EnvProbe( &culler, frusta, arraysize(frusta), cmd );
			GGTrees::GGTrees_Draw_EnvProbe( &culler, frusta, arraysize(frusta), cmd );
...
)
{
	...
	device->EventBegin("Postprocess_Snow", cmd);
	auto range = wiProfiler::BeginRangeGPU("Snow", cmd);

	device->BindComputeShader(&shaders[CSTYPE_POSTPROCESS_SNOW], cmd);
	...
	cb.xPPParams1.x = speed;
	cb.xPPParams1.y = offset;


	cb.xPPParams0.x = opacity;
	cb.xPPParams0.y = layers;
	cb.xPPParams0.z = depth;
	cb.xPPParams0.w = windiness;
	...
	)
{
	...
	device->EventBegin("Postprocess_Rain", cmd);
	auto range = wiProfiler::BeginRangeGPU("Rain", cmd);

	device->BindComputeShader(&shaders[CSTYPE_POSTPROCESS_RAIN], cmd);

	device->BindResource(CS, &input, TEXSLOT_ONDEMAND0, cmd);
	device->BindResource(CS, &raintexture, TEXSLOT_ONDEMAND1, cmd);
	device->BindResource(CS, &rainnormal, TEXSLOT_ONDEMAND2, cmd);

	const TextureDesc& desc = output.GetDesc();
	const TextureDesc& desctex = raintexture.GetDesc();
	...
	cb.xPPParams1.x = rainOffsetX;

	cb.xPPParams1.y = rainRefreactionScale;

	//PE: Perhaps not need this anymore, so might be able to use it for other things.
	//cb.xPPParams1.z = 1.0f / desctex.Width; //Texel size custom textures. width
	//cb.xPPParams1.w = 1.0f / desctex.Height; //Texel size custom textures. height

	cb.xPPParams0.x = opacity;
	cb.xPPParams0.y = rainScaleX;
	cb.xPPParams0.z = rainScaleY;
	cb.xPPParams0.w = rainOffsetY;
...
{
#ifdef ENABLE_TRANSPARENT_SHADOWS
	return TRANSPARENTSHADOWSENABLED;
#else
	return false;
#endif
}
