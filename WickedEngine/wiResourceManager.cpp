...
namespace wiResourceManager
{
	...
	std::shared_ptr<wiResource> Load(const std::string& name, uint32_t flags, const uint8_t* filedata, size_t filesize)
	{
		...
		if (success)
		{
			...
			else if (!resource->filedata.empty() && (flags & IMPORT_RETAIN_FILEDATA) == 0)
			{
				...
				//PE: As the resource is created and not destroyed.
				//PE: And resource->filedata has already been allocated with the filesize.
				//PE: resource->filedata.clear(); will not free mem used, but just set the capacity of the vector.
				//PE: To actually free the vector we need shrink_to_fit :) , that was a hard bug to find :)
	...
	// can't register resources anymore
	std::shared_ptr<wiResource> GetResource( const std::string& name, int create, int* pFound )
	{
